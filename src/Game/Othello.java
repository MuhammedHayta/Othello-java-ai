...
public class Othello {
    ...
    public static boolean isGameOver(int[] board) {
        ...
        return getValidMoves(board, 1).isEmpty() && getValidMoves(board, 2).isEmpty();
    }
    ...
    public static boolean isValidMove(int[] board, int x, int y, int player) {
        if (getPlaceOnBoard(board, x, y) != 0)
            return false;
        ...
        for (int i = 0; i < directions.length; i++) {
            ...
            if (nx >= 0 && nx < boardSize && ny >= 0 && ny < boardSize
                    && getPlaceOnBoard(board, nx, ny) == 3 - player) {
                ...
                if (nx >= 0 && nx < boardSize && ny >= 0 && ny < boardSize
                        && getPlaceOnBoard(board, nx, ny) == player) {
                    return true;
                }
            }
        }
